    ...
    while not FoundArifact:
        ...
        if Settings["WantedSet"] == GeneratedArifact.Set or Settings["WantedSet"] == "ANY":  # if correct set
            ...
            if Settings["WantedType"] == GeneratedArifact.Type or Settings["WantedType"] == "ANY":  # if correct type
                ...
                if (
                    Settings["WantedMainStat"] == GeneratedArifact.MainStat or Settings["WantedMainStat"] == "ANY"
                ):  # if correct main stat
...
FoundArifacts = (
    []
)  # list where the amount of artifacts that were pulled to obtain the wanted one is saved, each index is 1 simulation
...
print(
    f'[Artifacts of the correct set]-----------------------------({SimulationStats["TotalArifactsOfTheCorrectSet"]:,})<{round(SimulationStats["TotalArifactsOfTheCorrectSet"]/SimulationStats["TotalArifactsPulled"]*100,2)}%>'
)
print(
    f'[Artifacts of the correct piece type]----------------------({SimulationStats["TotalArifactsOfTheCorrectType"]:,})<{round(SimulationStats["TotalArifactsOfTheCorrectType"]/SimulationStats["TotalArifactsPulled"]*100,2)}%>'
)
print(
    f'[Artifacts of the correct main stat]-----------------------({SimulationStats["TotalArifactsWithCorrectMainStat"]:,})<{round(SimulationStats["TotalArifactsWithCorrectMainStat"]/SimulationStats["TotalArifactsPulled"]*100,2)}%>'
)
print(
    f'[Artifacts of the correct substats]------------------------({SimulationStats["TotalArifactsWithCorrectSubStats"]:,})<{round(SimulationStats["TotalArifactsWithCorrectSubStats"]/SimulationStats["TotalArifactsPulled"]*100,2)}%>'
)
print(
    f'[Artifacts of the correct substats and wanted values]------({SimulationStats["TotalArifactsWithCorrectSubStatsAndValues"]:,})<{round(SimulationStats["TotalArifactsWithCorrectSubStatsAndValues"]/SimulationStats["TotalArifactsPulled"]*100,2)}%>'
)
